...
// CORS for dev
app.use((0, cors_1.default)({ origin: ["http://localhost:4200"], credentials: false }));
...
app.get("/api/artworks", (req, res) => {
    ...
    if (q) {
        ...
        items = items.filter(a => (a.title + a.artist + a.medium + (a.tags || []).join()).toLowerCase().includes(needle));
    }
    res.json(items);
});
...
if (process.env.NODE_ENV === "production") {
    const angularDistPath = path_1.default.join(__dirname, "../dist/web");
